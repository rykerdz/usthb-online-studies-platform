...
public class UploadRecordController {
    ...
    UploadRecordController(User user, Authentication auth)
    {
        this.auth = auth;
        this.user = user;
    }
            ...
            if (type != null) {
                if (link != null) {
                    if(durationP.getText() != null){
                        // push
                        if (auth.uploadRecord(user.getClassroom_id(), user.getId(), date, type, link, durationP.getText(), timeP.getValue())) {
                            PopUp.showDialog("Successfully Uploaded the record!!", "Success");
                        }
                    }
                    else {
